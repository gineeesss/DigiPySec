<div>
    ...
    <!-- Resultados -->
    <div class="grid gap-8 md:grid-cols-2 lg:grid-cols-3" wire:key="posts-{{ md5($posts->toJson()) }}">
        @forelse($posts as $post)
            <livewire:blog.post-card
                :post="$post"
                :wire:key="'post-card-'.$post->id.'-'.now()->timestamp"
            />
        @empty
        ...
        <script>
            document.addEventListener('livewire:init', () => {
                Livewire.on('page-changed', () => {
                    // Forzar recarga suave después de paginación/filtros
                    window.Livewire.dispatch('refresh-posts');
                });
            });
        </script>
    @endpush
